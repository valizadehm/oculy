...
class SimpleViewerWorkspace(Workspace):
    ...
    def _post_setattr_selected_folder(self, old, new):
        ...
        self._watchdog_watch = self._watchdog.schedule(
            self._watchdog_handler, new
        )
    ...
    def _post_setattr_selected_file(self, old, new):
        ...
        if self._loader is not None:
            self._loader.path = os.path.join(
                self.selected_folder, self.selected_file
            )
        if self.auto_load:
            self.load_file()
